...
import { getTurndownService, htmlToMarkdown, elementToMarkdown } from './turndown-service';

function createDOM(html: string, baseUrl = 'https://example.com/page'): Document {
...
describe('turndown-service', () => {
  ...
  describe('Integration: Complex HTML to Markdown', () => {
    it('should handle mixed content with LaTeX and images', () => {
      ...
      expect(markdown).toContain('${E = mc^2}$');
      expect(markdown).toContain('![Einstein](https://example.com/einstein.jpg)');
      expect(markdown).toContain('$${\\frac{\\partial^2 u}{\\partial t^2} = c^2 \\nabla^2 u}$$');
    });
